...
- [x] Add a separate opt-in long-form native benchmark (`SPEACHY_RUN_LONGFORM_TRANSCRIPTION_BENCHMARK=1`) driven by a checked-in provenance manifest and a machine-local corpus directory; it writes versioned JSON evidence under ignored `web/test-results/` without copying source audio into the repository
